...
// ============================================================
// CUSTOMER COST MATRIX FUNCTIONS
// ============================================================
...
function addQuoteProduct(){
  ...
  if(!product){
    showToast('Enter a product','warn');
    return;
  }
  ...
  // Initialize quoteProducts if needed
  if(!S.quoteProducts)S.quoteProducts=[];

  // Add with empty lengths (duplicates allowed for different ship times)
  S.quoteProducts.push({
    product:product,
    lengths:{}
  });

  save('quoteProducts',S.quoteProducts);
  input.value='';
  input.focus();
  render();
}

// Remove a product from the quote list
function removeQuoteProduct(idx){
  if(!S.quoteProducts)return;
  S.quoteProducts.splice(idx,1);
  save('quoteProducts',S.quoteProducts);
  render();
}

// Toggle a length for a product
function toggleQuoteLength(idx,length){
  if(!S.quoteProducts||!S.quoteProducts[idx])return;
  if(!S.quoteProducts[idx].lengths)S.quoteProducts[idx].lengths={};
  S.quoteProducts[idx].lengths[length]=!S.quoteProducts[idx].lengths[length];
  save('quoteProducts',S.quoteProducts);
  render();
}

// Select all lengths for a product
function selectAllLengths(idx){
  if(!S.quoteProducts||!S.quoteProducts[idx])return;
  const lengths=['8','10','12','14','16','18','20'];
  S.quoteProducts[idx].lengths={};
  lengths.forEach(l=>S.quoteProducts[idx].lengths[l]=true);
  save('quoteProducts',S.quoteProducts);
  render();
}

// Update a field on a quote product (qty, ship, etc.)
function updateQuoteProductField(idx,field,value){
  if(!S.quoteProducts||!S.quoteProducts[idx])return;
  S.quoteProducts[idx][field]=value;
  save('quoteProducts',S.quoteProducts);
}
...
function clearQuoteProducts(){
  ...
  S.costMatrix={};
  save('quoteProducts',S.quoteProducts);
  save('costMatrix',S.costMatrix);
  render();
}

// Copy all costs to clipboard as HTML table
function copyAllCosts(){
  const matrix=S.costMatrix||{};
  const products=S.quoteProducts||[];
  const destination=S.qbCustomDest||'';
  const customerName=S.qbCustomer||'Customer';

  if(!Object.keys(matrix).length){
    showToast('No costs to copy','warn');
    return;
  }
  ...
  products.forEach(p=>{
    const lengths=['8','10','12','14','16','18','20'];
    lengths.forEach(len=>{
      if(!p.lengths||!p.lengths[len])return;
      const key=`${p.product}|${len}`;
      const cell=matrix[key];
      if(!cell||!cell.landedCost)return;
      items.push({
        product:p.product,
        length:len+"'",
        price:'$'+Math.round(cell.landedCost),
        qty:p.qty||'',
        ship:p.ship||'1-2 Weeks'
      });
    });
  });

  if(!items.length){
    showToast('No items to copy','warn');
    return;
  }
...
<div style="font-family:Arial,sans-serif;font-size:14px;">
  <p style="margin:0 0 5px 0;"><strong>SYP AVAILABILITY — ${customerName.toUpperCase()}</strong></p>
  <p style="margin:0 0 5px 0;">Delivered to: ${destination}</p>
  <p style="margin:0 0 15px 0;color:#666;">${dateStr}</p>
  <table style="border-collapse:collapse;width:100%;max-width:600px;">
    <thead>
      <tr style="background:#1a1a2e;color:#fff;">
        <th style="border:1px solid #333;padding:10px 12px;text-align:left;">Product</th>
        <th style="border:1px solid #333;padding:10px 12px;text-align:center;">Length</th>
        <th style="border:1px solid #333;padding:10px 12px;text-align:right;">Price</th>
        <th style="border:1px solid #333;padding:10px 12px;text-align:center;">Qty</th>
        <th style="border:1px solid #333;padding:10px 12px;text-align:center;">Ship</th>
      </tr>
    </thead>
    <tbody>`;
  ...
  items.forEach((item,i)=>{
    ...
    html+=`
      <tr style="background:${bg};">
        <td style="border:1px solid #ddd;padding:8px 12px;font-weight:600;">${item.product}</td>
        <td style="border:1px solid #ddd;padding:8px 12px;text-align:center;">${item.length}</td>
        <td style="border:1px solid #ddd;padding:8px 12px;text-align:right;font-weight:600;color:#2e7d32;">${item.price}</td>
        <td style="border:1px solid #ddd;padding:8px 12px;text-align:center;">${item.qty}</td>
        <td style="border:1px solid #ddd;padding:8px 12px;text-align:center;">${item.ship}</td>
      </tr>`;
  });
  ...
  <p style="margin:15px 0 0 0;font-size:12px;color:#666;">All prices per MBF, delivered to ${destination}.</p>
  <p style="margin:5px 0 0 0;font-size:12px;color:#666;">Quote valid for 24 hours. Subject to prior sale.</p>
</div>`;

  // Plain text fallback
  let text=`SYP AVAILABILITY — ${customerName.toUpperCase()}\n`;
  text+=`Delivered to: ${destination}\n${dateStr}\n\n`;
  text+=`PRODUCT\tLENGTH\tPRICE\tQTY\tSHIP\n`;
  items.forEach(item=>{
    text+=`${item.product}\t${item.length}\t${item.price}\t${item.qty}\t${item.ship}\n`;
  });
  text+=`\nAll prices per MBF, delivered to ${destination}.\nQuote valid for 24 hours. Subject to prior sale.`;
  ...
  navigator.clipboard.write([
    ...
    showToast('Quote table copied!','positive');
  }).catch(()=>{
    // Fallback to plain text
    navigator.clipboard.writeText(text).then(()=>{
      showToast('Quote copied (text)','positive');
    });
  });
}

// Load delivered cost matrix for selected customer
async function loadCustomerCostMatrix(){
  // Get destination from UI
  const destInput=document.getElementById('cost-matrix-dest');
  const destination=destInput?.value?.trim()||S.qbCustomDest||'';

  if(!destination){
    showToast('Enter a destination city first','warn');
    return;
  }

  // Build selected keys from quoteProducts
  const quoteProducts=S.quoteProducts||[];
  const selectedKeys=[];
  quoteProducts.forEach(p=>{
    Object.keys(p.lengths||{}).forEach(len=>{
      if(p.lengths[len])selectedKeys.push(`${p.product}|${len}`);
    });
  });

  if(!selectedKeys.length){
    showToast('Select some lengths first','warn');
    return;
  }

  // Set loading state
  S.costMatrixLoading=true;
  S.costMatrix={};
  render();

  // Extract unique products and lengths from selection
  const products=new Set();
  const lengths=new Set();
  selectedKeys.forEach(key=>{
    const[prod,len]=key.split('|');
    products.add(prod);
    lengths.add(len);
  });

  // Collect all mill quotes from Mill Intel and local
  let allQuotes=[];

  // Try Mill Intel API first
  if(typeof miLoadLatestQuotes==='function'){
    try{
      const miQuotes=await miLoadLatestQuotes({});
      if(Array.isArray(miQuotes))allQuotes=allQuotes.concat(miQuotes);
    }catch(e){
      console.warn('Mill Intel fetch failed:',e);
    }
  }

  // Also include local mill quotes from S.millQuotes
  if(S.millQuotes&&S.millQuotes.length){
    allQuotes=allQuotes.concat(S.millQuotes);
  }
  ...
  for(const key of selectedKeys){
    ...
    // Find all quotes matching this product/length
    const matching=allQuotes.filter(q=>{
        const qProd=normalizeProduct(q.product);
        const qLen=normalizeLength(q.length||'RL');

        // Product must match
        if(qProd!==prod)return false;

        // Length: RL matches any, or exact match
        if(len==='RL')return qLen==='RL'||!qLen;
        if(qLen==='RL'||!qLen)return true; // RL quotes can fill specific lengths
        return qLen===len;
      });

      if(!matching.length)continue;

      // Calculate landed cost for each and find best
      let bestCost=null;
      let bestQuote=null;

      for(const q of matching){
        const millName=q.mill_name||q.mill||'';
        const fobPrice=parseFloat(q.price)||0;
        if(!fobPrice)continue;

        // Get mill origin from MILL_DIRECTORY or quote
        let origin='';
        const dirEntry=typeof MILL_DIRECTORY!=='undefined'?MILL_DIRECTORY[millName]:null;
        if(dirEntry){
          origin=`${dirEntry.city}, ${dirEntry.state}`;
        }else{
          origin=q.city&&q.state?`${q.city}, ${q.state}`:q.city||'';
        }

        if(!origin){
          // Try to extract from mill name (e.g., "Company - City")
          const parts=millName.split(' - ');
          if(parts.length>1){
            const city=parts[parts.length-1];
            // Look up state from MI_MILL_CITIES if available
            const state=typeof MI_MILL_CITIES!=='undefined'?MI_MILL_CITIES[city.toLowerCase()]:'';
            origin=state?`${city}, ${state}`:city;
          }
        }

        if(!origin)continue;

        // Get miles and freight
        let miles=getLaneMiles(origin,destination);

        if(!miles){
          // Track missing lanes for batch lookup
          missingLanes.add(JSON.stringify({origin,dest:destination}));
          continue;
        }

        const isMSR=prod.includes('MSR');
        const freightPerMBF=calcFreightPerMBF(miles,origin,isMSR);
        const landedCost=fobPrice+freightPerMBF;

        if(bestCost===null||landedCost<bestCost){
          bestCost=landedCost;
          bestQuote={
            mill:millName,
            origin,
    ...
    if(bestQuote){
      matrix[key]=bestQuote;
    }
  }

  // If we have missing lanes, try to fetch them
  if(missingLanes.size>0){
    const laneArr=Array.from(missingLanes).map(s=>JSON.parse(s));
    showToast(`Looking up ${laneArr.length} freight lanes...`,'info');

    try{
      await lookupMileageWithAPI(laneArr);

      // Re-run the matrix calculation with new lanes
      for(const key of selectedKeys){
        if(matrix[key])continue; // Already have best
        const[prod,len]=key.split('|');
        ...
        for(const q of matching){
          ...
          let origin='';
          const dirEntry=typeof MILL_DIRECTORY!=='undefined'?MILL_DIRECTORY[millName]:null;
          if(dirEntry)origin=`${dirEntry.city}, ${dirEntry.state}`;
          else origin=q.city&&q.state?`${q.city}, ${q.state}`:q.city||'';
          ...
          if(!matrix[key]||landedCost<matrix[key].landedCost){
            matrix[key]={mill:millName,origin,fobPrice,freightPerMBF,landedCost,miles,date:q.date||'',length:q.length||'RL'};
          }
        }
      }
    }catch(e){
      console.warn('Lane lookup failed:',e);
    }
  }

  // Save matrix to state
  S.costMatrix=matrix;
  S.costMatrixLoading=false;
  save('costMatrix',S.costMatrix);

  const filledCells=Object.keys(matrix).length;
  showToast(`Loaded ${filledCells} cost cells`,'positive');
  render();
}

// Show detailed cost breakdown when clicking a matrix cell
function showCostDetail(product,length){
  const key=`${product}|${length}`;
  const cell=S.costMatrix?.[key];

  if(!cell){
    showToast('No cost data for this cell','warn');
    return;
  }

  const destination=S.qbCustomDest||'';
  const margin25=Math.round(cell.landedCost+25);
  const margin50=Math.round(cell.landedCost+50);
  const margin75=Math.round(cell.landedCost+75);

  // Find alternative mills for this product/length
  const products=['2x4#2','2x6#2','2x8#2','2x10#2','2x12#2','2x4#1','2x6#1','2x8#1'];
  let allQuotes=[...(S.millQuotes||[])];

  const alternatives=allQuotes.filter(q=>{
    const qProd=normalizeProduct(q.product);
    const qLen=normalizeLength(q.length||'RL');
    if(qProd!==product)return false;
    if(length==='RL')return qLen==='RL'||!qLen;
    if(qLen==='RL'||!qLen)return true;
    return qLen===length;
  }).map(q=>{
    const millName=q.mill_name||q.mill||'';
    const fobPrice=parseFloat(q.price)||0;
    if(!fobPrice)return null;
    ...
    const miles=origin?getLaneMiles(origin,destination):null;
    const freightPerMBF=miles?calcFreightPerMBF(miles,origin,product.includes('MSR')):0;
    const landedCost=fobPrice+(miles?freightPerMBF:0);

    return{mill:millName,origin,fobPrice,freightPerMBF,landedCost,miles,hasMiles:!!miles};
  }).filter(Boolean).sort((a,b)=>a.landedCost-b.landedCost).slice(0,5);

  const html=`
    <div class="modal-overlay" onclick="this.remove()">
      <div class="modal" onclick="event.stopPropagation()" style="max-width:500px">
        <div class="modal-header">
          <span class="modal-title">${product} ${length==='RL'?'RL':length+"'"}</span>
        ...
        <div class="modal-body">
          <div style="background:var(--panel-alt);padding:16px;border-radius:8px;margin-bottom:16px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
              <div style="font-size:10px;color:var(--muted);text-transform:uppercase">Best Delivered Cost</div>
              <div style="font-size:24px;font-weight:700;color:var(--positive);font-family:var(--mono)">$${Math.round(cell.landedCost)}</div>
            </div>
            <div style="font-size:13px;font-weight:600;margin-bottom:4px">${cell.mill}</div>
            <div style="font-size:11px;color:var(--muted)">${cell.origin} → ${destination}</div>
          </div>

          <table style="width:100%;font-size:11px;margin-bottom:16px">
            <tr><td style="color:var(--muted)">Mill FOB</td><td style="text-align:right;font-family:var(--mono)">$${Math.round(cell.fobPrice)}</td></tr>
            <tr><td style="color:var(--muted)">Freight (${cell.miles} mi)</td><td style="text-align:right;font-family:var(--mono)">$${Math.round(cell.freightPerMBF)}</td></tr>
            <tr style="border-top:1px solid var(--border)"><td style="font-weight:600">Landed Cost</td><td style="text-align:right;font-weight:700;font-family:var(--mono);color:var(--positive)">$${Math.round(cell.landedCost)}</td></tr>
          </table>

          <div style="background:var(--panel);padding:12px;border-radius:6px;margin-bottom:16px">
            <div style="font-size:10px;color:var(--muted);text-transform:uppercase;margin-bottom:8px">Sell Price Suggestions</div>
            <div style="display:flex;gap:12px">
              <div style="flex:1;text-align:center;padding:8px;background:var(--bg);border-radius:4px">
                <div style="font-size:9px;color:var(--muted)">+$25</div>
                <div style="font-weight:600;font-family:var(--mono)">$${margin25}</div>
              </div>
              <div style="flex:1;text-align:center;padding:8px;background:var(--bg);border-radius:4px">
                <div style="font-size:9px;color:var(--muted)">+$50</div>
                <div style="font-weight:600;font-family:var(--mono)">$${margin50}</div>
              </div>
              <div style="flex:1;text-align:center;padding:8px;background:var(--bg);border-radius:4px">
                <div style="font-size:9px;color:var(--muted)">+$75</div>
                <div style="font-weight:600;font-family:var(--mono)">$${margin75}</div>
              </div>
            </div>
          </div>

          ${alternatives.length>1?`
          <div style="font-size:10px;color:var(--muted);text-transform:uppercase;margin-bottom:8px">Alternative Sources</div>
          <table style="width:100%;font-size:10px">
            <thead><tr style="color:var(--muted)"><th style="text-align:left">Mill</th><th style="text-align:right">FOB</th><th style="text-align:right">Frt</th><th style="text-align:right">Landed</th></tr></thead>
            <tbody>
              ${alternatives.slice(1,5).map(a=>`
                <tr style="opacity:${a.hasMiles?1:0.5}">
                  <td style="padding:4px 0">${a.mill.length>20?a.mill.substring(0,20)+'...':a.mill}</td>
                  <td style="text-align:right;font-family:var(--mono)">$${Math.round(a.fobPrice)}</td>
                  <td style="text-align:right;font-family:var(--mono)">${a.hasMiles?'$'+Math.round(a.freightPerMBF):'?'}</td>
                  <td style="text-align:right;font-family:var(--mono)">${a.hasMiles?'$'+Math.round(a.landedCost):'—'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
        ...
        <div class="modal-footer">
          <button class="btn btn-default" onclick="this.closest('.modal-overlay').remove()">Close</button>
          <button class="btn btn-primary" onclick="copyProductQuote('${product}','${length}')">Copy Quote</button>
        </div>
      </div>
    </div>
  `;
...
// Copy a single product quote to clipboard
function copyProductQuote(product,length){
  const key=`${product}|${length}`;
  const cell=S.costMatrix?.[key];
  if(!cell)return;
  ...
  const destination=S.qbCustomDest||'';
  const customerName=S.qbCustomer||'Customer';
  const sellPrice=Math.round(cell.landedCost+50); // Default +$50 margin

  const text=`${product} ${length==='RL'?'RL':length+"'"}: $${sellPrice} delivered ${destination}\n(via ${cell.mill})`;
